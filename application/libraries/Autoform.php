 ...
 * @since     Version 3.0
 * @filesource
 * @modifications: brian.muhumuza@gmail.com
 */
class Autoform {
	...
	private $after = array();
	private $before_field_only = array(); // Author: brian.muhumuza@gmail.com
	private $after_field_only = array(); // Author: brian.muhumuza@gmail.com
	private $help_text = array(); // Author: brian.muhumuza@gmail.com
	public $inline_errors = TRUE;
	public $error_class = 'error'; // Author: brian.muhumuza@gmail.com
	...
	public function set($field_id, $attr=array()) 
	{
		...
		if (isset($this->fields->$field_id)) 
		{
			foreach ($attr as $key=>$value) 
			{
				...
				// start additions brian.muhumuza@gmail.com
				elseif ($key=='after_field_only') 
				{
					$this->after_field_only($field_id, $value);
				}
				elseif ($key=='before_field_only') 
				{
					$this->before_field_only($field_id, $value);
				} 
				// end additions
				else {
	 ...
	 * 
	 * Set attributes on all or specified fields
	 *
	 * @param Array $attr
	 * @param Array $fields [optional] - if no fields are passed, attributes will be set on all fields
	 * @return Array of Objects
	 */
	public function set_all($attr, $fields=null) {
		if ($fields == null) {
			$fields = array();
	 ...
	 * Author: brian.muhumuza@gmail.com
	 *
	 * Add a string before a field but excluding the label
	 * $side can be start or end and sets the string at 
	 * that position of the existing string
	 * 
	 * @param String $field_id
	 ...
	 * @return Object
	 */
	public function before_field_only($field_id, $input, $side='start') 
	{

		if (isset($this->before_field_only[$field_id])) 
		{
			// add to existing string
			if ($side=='start') 
			{
				// add in front of existing
				$this->before_field_only[$field_id] = $input . $this->before_field_only[$field_id];
			}
			else {
				// add after existing
				$this->before_field_only[$field_id] .= $input;
			}
		}
		else {
			// create new string
	 ...
	 * @return Object
	 */
	public function after_field_only($field_id, $input, $side='end') 
	{

		if (isset($this->after_field_only[$field_id])) 
		{
			// add to existing string
			if ($side=='start') 
			{
				// add in front of existing
				$this->after_field_only[$field_id] = $input . $this->after_field_only[$field_id];
			}
			else {
				// add after existing
				$this->after_field_only[$field_id] .= $input;
			}
		}
		else {
			// create new string
	...
	private function validate($fields) 
	{
		...
		{
			// modification start - brian.muhumuza@gmail.com
#			if ($this->inline_errors==TRUE) 
#			{
#				$field->label->content = trim(form_error($field->name));
#			}
			// modification end - brian.muhumuza@gmail.com

			// add error class to label
			if (isset($field->label->extra['class']) && !preg_match('/error/', $field->label->extra['class'])) 
			{ 
				$field->label->extra['class'] .= ' '.$this->error_class; // modified - brian.muhumuza@gmail.com
			}
			...
			{ 
				$field->class .= ' '.$this->error_class; // modified - brian.muhumuza@gmail.com
			}
	...
	private function build($field) 
	{
		...
		// modification start - brian.muhumuza@gmail.com
		if (isset($this->before_field_only[$field->id]))
		{
			$before_field_only = $this->before_field_only[$field->id];
		}
		else
		{
			$before_field_only = '';
		...
		if (isset($this->after_field_only[$field->id]))
		{
			$after_field_only = $this->after_field_only[$field->id];
		}
		else
		{
			$after_field_only = '';
		...
		if (isset($this->help_text[$field->id]))
		{
			$help_text = '<small>' . $this->help_text[$field->id] . '</small>';
		}
		else
		{
			$help_text = '';
		...
		switch ($field->type) 
		{
			...
			case 'dropdown':
				...
				{      		
					...
					{
						case 'wrap':
							$output .= form_label($field->label->content . $before_field_only . $this->dropdown($field) . $after_field_only . $help_text, $field->id, $field->label->extra); //mod - brian.muhumuza@gmail.com
						break;
						case 'right':
							$output .= $before_field_only . $this->dropdown($field) . $after_field_only . $help_text; //mod - brian.muhumuza@gmail.com
			...
			case 'radio':
				...
				if ($field->label->content!='') 
				{
					...
					{
						case 'wrap':
							$output .= form_label($field->label->content . $before_field_only . $this->checked_input($field) . $after_field_only . $help_text, $field->id, $field->label->extra); //mod - brian.muhumuza@gmail.com
						break;
						case 'right':
							$output .= $before_field_only . $this->checked_input($field) . $after_field_only . $help_text; //mod - brian.muhumuza@gmail.com
							$output .= ' '.form_label($field->label->content, $field->id, $field->label->extra);
						break;
						case 'left':
						default:
			...
			case 'textarea':
				...
				if ($field->label->content!='' && $field->type!='hidden') 
				{
					...
					{
						case 'wrap':
							$output .= form_label($field->label->content . $before_field_only . $this->textarea($field) . $after_field_only . $help_text, $field->id, $field->label->extra); //mod - brian.muhumuza@gmail.com
						break;
						case 'right':
							$output .= $before_field_only . $this->textarea($field) . $after_field_only . $help_text; //mod - brian.muhumuza@gmail.com
							$output .= ' '.form_label($field->label->content, $field->id, $field->label->extra);
						break;
						case 'left':
						default:
			...
			default:
				...
				if ($field->label->content!='' && $field->type!='hidden') 
				{
					...
					{
						case 'wrap':
							$output .= form_label($field->label->content . $before_field_only . $this->input($field), $field->id . $after_field_only . $help_text, $field->label->extra); //mod - brian.muhumuza@gmail.com
						break;
						case 'right':
							$output .= $before_field_only . $this->input($field) . $after_field_only . $help_text; //mod - brian.muhumuza@gmail.com
							$output .= ' '.form_label($field->label->content, $field->id, $field->label->extra);
						break;
						case 'left':
						default:
		...
		if (form_error($field->name))
		{
			if ($this->inline_errors==TRUE)
			{
				$field->label->content = '"'.trim(form_error($field->name)).'"<br>'.$field->label->content;
			}
		}
		// modification end - brian.muhumuza@gmail.com
	...
	private function dropdown($field) 
	{
		...
		// add field value to options if we have no options // added: brian.muhumuza@gmail.com
		if (strlen($field->value) > 0 && count($options) == 0) {  // added: brian.muhumuza@gmail.com
			$options[$field->value] = $field->value; // added: brian.muhumuza@gmail.com
		} // added: brian.muhumuza@gmail.com
	...
	public function fields($field_order=FALSE) 
	{
		...
		if (is_array($field_order)) 
		{
			foreach ($field_order as $field_id) 
			{
				...
				// validate field
				$this->validate($this->fields->$field_id); // added - brian.muhumuza@gmail.com
				// generate field
	 ...
	 * Author: brian.muhumuza@gmail.com
	 *
	 * Wrap each field (excluding the label) in $before & $after Strings
	 * 
	 * @param String $before
	 * @param String $after
	 * @param Array $fields [optional] an array of fields to wrap. other fields will be ignored. if null, wrap all fields
	 * @param Array $exceptions [optional] an array of fields to ignore
	 * @param Bool $include_hidden [optional] a boolean whether to include hidden fields or not
	 * 
	 */
	public function wrap_each_field_only($before = '', $after = '', $fields = NULL, $exceptions = array(), $include_hidden = FALSE) 
	{
		...
		foreach ($this->fields as $field) 
		{
			...
			$this->before_field_only($field->id, $before);
			$this->after_field_only($field->id, $after);
		}
	 ...
	 * 
	 */
	public function set_help_text($field_id, $help_text)
	{
		if (isset($this->fields->$field_id)) // check field exists
		{
			$this->help_text[$field_id] = $help_text;
		}
	}
